 ...
 * All rights reserved.                                                      *
 *                                                                           *
 * This file is part of the HDF5 TRACKER Virtual File Driver. The full        * 
...
/*
 * Error handling for the TRACKER VFD
 */

#ifndef H5FDtracker_vfd_err_h
#define H5FDtracker_vfd_err_h
...
extern hid_t H5FDtracker_vfd_err_stack_g;
extern hid_t H5FDtracker_vfd_err_class_g;

#define H5FD_TRACKER_VFD_ERR_CLS_NAME "HDF5 TRACKER VFD"
#define H5FD_TRACKER_VFD_ERR_LIB_NAME "HDF5 TRACKER VFD"
#define H5FD_TRACKER_VFD_ERR_VER      "1.0.0"
 ...
 */
#define H5FD_TRACKER_VFD_GOTO_ERROR(err_major, err_minor, ret_val, ...)      \
  do {                                                                  \
    unsigned is_v2_err;                                                 \
    union {                                                             \
    ...
    if (  (is_v2_err && err_func.err_func_v2) ||                        \
      ...
      if (H5FDtracker_vfd_err_stack_g >= 0 && H5FDtracker_vfd_err_class_g >= 0) { \
        H5Epush2(H5FDtracker_vfd_err_stack_g, __FILE__, __func__, __LINE__,  \
                 H5FDtracker_vfd_err_class_g, err_major, err_minor, __VA_ARGS__); \
      } else {                                                          \
 ...
 * and v2 errors)
 */
#define H5FD_TRACKER_VFD_DONE_ERROR(err_major, err_minor, ret_val, ...)      \
  do {                                                                  \
    unsigned is_v2_err;                                                 \
    union {                                                             \
      H5E_auto1_t err_func_v1;                                          \
...
#define PRINT_ERROR_STACK                                               \
  do {                                                                  \
    ...
    if (  (is_v2_err && err_func.err_func_v2) ||                        \
          (!is_v2_err && err_func.err_func_v1) ) {                      \
      if ((H5FDtracker_vfd_err_stack_g >= 0) &&                         \
          (H5Eget_num(H5FDtracker_vfd_err_stack_g) > 0)) {              \
        H5Eprint2(H5FDtracker_vfd_err_stack_g, NULL);                   \
        H5Eclear2(H5FDtracker_vfd_err_stack_g);                         \
      }                                                                 \
    }                                                                   \
  } while (0)

#define H5FD_TRACKER_VFD_SYS_GOTO_ERROR(err_major, err_minor, ret_val, str)  \
  do {                                                                  \
    int myerrno = errno;                                                \
    H5FD_TRACKER_VFD_GOTO_ERROR(err_major, err_minor, ret_val,               \
 ...
 */
#define H5FD_TRACKER_VFD_GOTO_DONE(ret_val)          \
  do {                                          \
    ret_value = ret_val;                        \
    goto done;                                  \
  } while (0)
 ...
 */
#define H5FD_TRACKER_VFD_FUNC_LEAVE_API              \
  do {                                          \
    PRINT_ERROR_STACK;                          \
    return ret_value;                           \
  } while (0)
 ...
 */
#define H5FD_TRACKER_VFD_FUNC_LEAVE                  \
  do {                                          \
    return ret_value;                           \
  } while (0)
