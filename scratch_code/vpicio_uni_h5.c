...
{
    printf("Usage: %s /path/to/file #timestep sleep_sec enable_dlife_logging /path/to/dlife_log_file\n", name);
}

hid_t fileaccess_mod(const char* log_file_path){
    hid_t fapl_id = H5I_INVALID_HID;
    htri_t connector_is_registered;    
    H5VL_datalife_info_t dlife_vol_info;
    //void *vol_info = NULL;
...
//    if(fapl_id == H5P_DEFAULT);
    ...
    dlife_vol_info.under_vol_id = under_vol_id;
    dlife_vol_info.under_vol_info = under_vol_info;

    hid_t connector_id = -1;

    
    if((connector_is_registered = H5VLis_connector_registered_by_name("datalife")) < 0){
        ...
        /* Retrieve the ID of the already-registered VOL connector */
         if((connector_id = H5VLget_connector_id_by_name("datalife")) < 0){
            printf("can not get vol id of datalife \n");
             goto error;
         }
    } 
    else {//not registed        
    ...
    dlife_vol_info.dlife_file_path = log_file_path;//"my_dlife_file";
    dlife_vol_info.dlife_level = 2; //0/1 for print, 2 for file only, 3 for both.
    dlife_vol_info.dlife_line_format = "";

    //printf("datalife data path: %s\n", dlife_vol_info.dlife_file_path);
    if(H5Pset_vol(fapl_id, connector_id, &dlife_vol_info) < 0){
