...
H5VL_datalife_t * _dset_wrap_under(void *under, H5VL_datalife_t *upper_o,
    hid_t blob_id, size_t size, void** req);
blob_dlife_info_t * add_blob_node(file_dlife_info_t *file_info, void * blob_id, 
    size_t size, H5O_token_t token);

static H5VL_datalife_blob_t *H5VL_datalife_new_blob_obj(dlife_helper_t* helper);

/*-------------------------------------------------------------------------
 * Function:    H5VL__datalife_new_blob_obj
 *
 ...
 *-------------------------------------------------------------------------
 */
static H5VL_datalife_blob_t *
H5VL_datalife_new_blob_obj()
{
//    unsigned long start = get_time_usec();
    H5VL_datalife_blob_t *new_blob_obj;

    // assert(helper);

    new_blob_obj = (H5VL_datalife_blob_t *)calloc(1, sizeof(H5VL_datalife_blob_t));
    // new_blob_obj->dlife_helper = helper;
    new_blob_obj->my_type = 123;
    ptr_cnt_increment(new_blob_obj->dlife_helper);
    //TOTAL_DLIFE_OVERHEAD += (get_time_usec() - start);
    return new_blob_obj;
} /* end H5VL__datalife_new_blob_obj() */

blob_dlife_info_t *new_blob_info(file_dlife_info_t *root_file, 
    dataset_dlife_info_t *root_dset, H5O_token_t token, 
    size_t size, void *blob_id)
{
    // const char *name, H5O_token_t token

    blob_dlife_info_t *info;
    const char * name = (const char *) root_dset->obj_info.name;

    info = (blob_dlife_info_t *)calloc(1, sizeof(blob_dlife_info_t));
...
blob_dlife_info_t * new_blob_dlife_info(file_dlife_info_t * file_info,
    dataset_dlife_info_t * dset_info, H5O_token_t token,
    size_t size, void *blob_id)
{
    ...
    // TODO(candice): handel parallel case for casting to dataset_dlife_info_t?
    ...
    size_t size, H5O_token_t token)
{
    ...
    if(!cur) {
        ...
        cur = new_blob_dlife_info(file_info, dset_info, token, size, blob_id);
    ...
    hid_t blob_id, size_t size, void** req)
{
    ...
    if (under) {
        ...
        obj->generic_dlife_info = add_blob_node(under, blob_id, size, token);
