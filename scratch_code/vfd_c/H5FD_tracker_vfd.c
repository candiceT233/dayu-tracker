...
#include "H5FD_tracker_vfd_log_c.h"
// extern "C" {
 ...
 */
herr_t
H5Pset_fapl_tracker_vfd(hid_t fapl_id, hbool_t logStat, size_t page_size) {
...
H5FD__tracker_vfd_open(const char *name, unsigned flags, hid_t fapl_id,
  ...
  H5E_BEGIN_TRY {
      fa = (const H5FD_tracker_vfd_fapl_t*)H5Pget_driver_info(fapl_id);
  }
  H5E_END_TRY;

  if (!fa || (H5P_FILE_ACCESS_DEFAULT == fapl_id)) {
      char config_str_buf[128];
      int config_str_len = H5Pget_driver_config_str(fapl_id, config_str_buf, sizeof(config_str_buf));
      if (config_str_len < 0) {
          ...
          if (token != NULL && (strcmp(token, "true") == 0 || strcmp(token, "TRUE") == 0 || strcmp(token, "True") == 0)) {
              new_fa.logStat = 1; // Set logStat to true
          }
  ...
  if(TKR_HELPER_VFD == NULL){
    ...
    parseEnvironmentVariable(file_path, sizeof(file_path));
    ...
    while (size > 0) {
        ...
        if (-1 == bytes_read) { /* error */
            ...
            H5FD_TRACKER_VFD_GOTO_ERROR(H5E_IO, H5E_READERROR, FAIL,
                        ...
                        HDctime(&mytime), file->filename_, file->fd, myerrno, strerror(myerrno), buf,
